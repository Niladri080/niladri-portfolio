...
const LightPillar: React.FC<LightPillarProps> = ({
  ...
  // null = not checked yet, true = supported, false = unsupported
  const [webGLSupported, setWebGLSupported] = useState<boolean | null>(null);

  // Check WebGL support (client-only). We set an explicit boolean when the check completes
  useEffect(() => {
    ...
    try {
      ...
      if (gl) {
        try {
          // Accessing a parameter is a simple way to detect invalid/blocked contexts
    ...
    if (!supported) console.warn('WebGL is not supported in this browser');
  }, []);

  useEffect(() => {
    // Wait until we've confirmed WebGL is available before attempting to create a renderer
  ...
  // If check hasn't completed, render nothing to avoid a flash / renderer attempt
